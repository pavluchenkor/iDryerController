...
//!!hello huhen https://github.com/huhen/iDryerController/
...
void isr()
{
    ...
    testTIMER_COUNT++;
#endif
    if ((state == DRY || state == STORAGE || state == AUTOPID) && Servo1.state != MOVE)
    {
        if (lastDim != dimmer)
        {
    ...
    do
    {
        ...
        for (uint8_t i = 0; i < maxPos; i++)
        {
                ...
                eeprom_read_word(&menuVal[subMenuM.membersID[subMenuM.linesToScreen[i]]]))
            {
                ...
                oled.drawUTF8(95, (i + 2) * LINE_HIGHT, val);
            }
...
void displayPrintMode()
{
    ...
    do
    {
        ...
        // //!! дебаг выключения кулера
        // sprintf(val, "%3hu", iDryer.data.flagTimeCounter ? map(iDryer.data.setFan, 0, 100, 0, 255) : 255);
        // sprintf(val, "%4d", analogRead(NTC_PIN));
...
void setup()
{
    ...
    pid.SetTunings(double(iDryer.data.Kp), double(iDryer.data.Ki), double(iDryer.data.Kd), PID_TYPE);
...
void loop()
{
    ...
    case STORAGE:
        ...
        DEBUG_PRINT(402);
        getdataAndSetpoint();
        if (((uint8_t)iDryer.data.bmeHumidity >= iDryer.data.setHumidity || iDryer.data.setTemp <= (uint8_t)iDryer.data.bmeTemp) && iDryer.data.flag)
        {
            DEBUG_PRINT(403);
            // iDryer.data.flag = 1;
            Input = double(iDryer.data.ntcTemp);
        ...
        DEBUG_PRINT(407);
        if (((uint8_t)iDryer.data.bmeHumidity > iDryer.data.setHumidity + HUMIDITY_HYSTERESIS || iDryer.data.setTemp - TEMP_HYSTERESIS > (uint8_t)iDryer.data.bmeTemp) && !iDryer.data.flag)
        {
            DEBUG_PRINT(408);
            iDryer.data.flag = 1;
#ifdef v220V
...
ISR(WDT_vect)
{
    ...
    // delay(10);
    // //!! проверить!!
    // // Установка битов WDE и WDCE для изменения настроек сторожевого таймера
    // WDTCSR |= (1 << WDE) | (1 << WDCE);
    // // Для активации перезагрузки
    // WDTCSR = 0x08;
    // // Сброс сторожевого таймера и перезагрузка
    // wdt_reset();
    // //!! проверить!!
    ...
    // Ждем готовности для записи в EEPROM
    while (EECR & (1 << EEPE));
    
    // Записываем данные в EEPROM
    EEAR = reinterpret_cast<uint16_t>(&ERROR_CODE);
    EEDR = static_cast<uint8_t>(WDT_ERROR);
    EECR |= (1 << EEMPE);
    EECR |= (1 << EEPE);
...
void pwm_test()
{
    ...
    for (uint8_t i = 0; i < 11; i++)
    {
        ...
        while (k > 0)
        {
            ...
            {
                oled.setFont(u8g2_font);
                sprintf(serviceString, "%s %6hu", printMenuItem(&serviceTxt[DEF_T_FREQUENCY]), frequency);
...
void setSpool(uint8_t spool)
{
    ...
    for (uint8_t i = 0; i < 4; i++)
    {
        ...
        {
            oled.setFont(u8g2_font);
            sprintf(serviceString, "%s %1d", printMenuItem(&serviceTxt[DEF_T_SPOOL]), spool + 1);
            oled.drawUTF8((128 - oled.getUTF8Width(serviceString)) / 2, LINE_HIGHT * 2, serviceString);
    ...
    do
    {
        ...
        sprintf(serviceString, "%s %1d", printMenuItem(&serviceTxt[DEF_T_SPOOL]), spool + 1);
        oled.drawUTF8((128 - oled.getUTF8Width(serviceString)) / 2, LINE_HIGHT * 2, serviceString);
        sprintf(serviceString, "%s", printMenuItem(&serviceTxt[DEF_T_SETUP]));
        oled.drawUTF8((128 - oled.getUTF8Width(serviceString)) / 2, LINE_HIGHT * 3, serviceString);
    } while (oled.nextPage());
    delay(1000);
    ...
    for (uint8_t i = 0; i < 10; i++)
    {
        ...
        do
        {
            ...
            sprintf(serviceString, "%s %1d", printMenuItem(&serviceTxt[DEF_T_SPOOL]), spool + 1);
            oled.drawUTF8((128 - oled.getUTF8Width(serviceString)) / 2, LINE_HIGHT * 1, serviceString);
            sprintf(serviceString, "%s", printMenuItem(&serviceTxt[DEF_T_PUT]));
            oled.drawUTF8((128 - oled.getUTF8Width(serviceString)) / 2, LINE_HIGHT * 2, serviceString);
...
void filamentCheck()
{
    ...
    if (hx711Multi.getMassMulti(hx711Multi.sensorNum) < ALERT_MASS &&
        hx711Multi.getMassMulti(hx711Multi.sensorNum) > FILAMENT_SENSOR_MASS)
    {
        piii(20);
    }

    if (hx711Multi.getMassMulti(hx711Multi.sensorNum) < FILAMENT_SENSOR_MASS)
    {
        digitalWrite(FILAMENT_SENSOR, 0);
        piii(50);
    }
